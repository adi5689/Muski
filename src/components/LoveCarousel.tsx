...
const AUTO_SCROLL_DELAY = 13000; // 13 seconds
...
const LoveCarousel = () => {
  ...
  const totalSlides = 1 + reasons.length + 1; // Cover + Reasons + End
  ...
  const onSelect = useCallback(() => {
    ...
    // Show progress only for reason slides
    setShowProgress(index > 0 && index <= reasons.length);

    // Open love letter overlay on last slide
  ...
  useEffect(() => {
    if (!emblaApi || showLetterSection) return;
  ...
  }, [emblaApi, totalSlides, showLetterSection]);

  /* ---------------- Handlers ---------------- */

  const handleStart = () => scrollTo(1);
    ...
    <div className="relative h-screen w-full overflow-hidden">
      ...
      <div ref={emblaRef} className="h-full w-full overflow-hidden touch-pan-y">
        <div className="flex flex-col h-full">
          {/* Cover Slide */}
          <div className="flex-[0_0_100%] h-screen">
            <CoverSlide onStart={handleStart} />
          </div>

          {/* Reason Slides */}
          {reasons.map((reason) => (
            <div key={reason.number} className="flex-[0_0_100%] h-screen">
              <ReasonSlide
      ...
      <div className="fixed right-4 sm:right-6 top-1/2 -translate-y-1/2 z-40 flex flex-col gap-3">
        {currentSlide > 0 && (
          <button
            onClick={scrollPrev}
            className="p-3 rounded-full glass-card shadow-romantic hover:bg-card transition"
            aria-label="Previous slide"
          >
            <ChevronUp className="w-5 h-5 text-muted-foreground hover:text-primary transition-colors" />
          </button>
        )}
